...
/* GPS Waypoints */
int waypoint_num = 0;
int special_waypoint_num = 3; // ALWAYS 1 LESS 'cause arrays start at 0, not 1!
const int waypoint_total = 14;	// <- should always be the same number of GPS waypoints
double gps_array[14][2] = {{40.065199508645286, -105.20982731314206}, // 1st corner
...
//		delay(1000);
		compass();
		initial_compass = compass_heading;
		initial_conditions = false;
	}
	gyro_heading = 360.0 - angle*360.0/GYRO_CAL + initial_compass;
	if(gyro_heading > 360.0) gyro_heading = gyro_heading - 360.0;
}

void set_speed(void) {	// test this and delete the delays after it works
		if(waypoint_distance <= 20)		speed.write(110);
		if(waypoint_distance > 20)		speed.write(120);
	return;
}
...
void waypoint(void) {				// Distance and angle to next waypoint
	...
	if(waypoint_num == special_waypoint_num) waypoint_heading = 288.0;
	...
	if(waypoint_distance < 10) waypoint_num++;
...
double servo_angle;
	...
	angle_diff = gyro_heading - waypoint_heading;
...
void LCD_data_log() {				// LCD Data Logging
	...
	lcd2.clear();
							lcd2.print("Gyro: "); lcd2.print(gyro_heading,1);
