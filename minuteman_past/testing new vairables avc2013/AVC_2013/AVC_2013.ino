...
int x_wp[WAYPOINT_COUNT], y_wp[WAYPOINT_COUNT];
double x=0, y=0;
...
void update_position(){
	//calculate position
	x += sin((angle + angle_last) * 3.14159/360.0);
	y += cos((angle + angle_last) * 3.14159/360.0);
	angle_last = angle;
	angle_target = atan2(((double)x_wp[wpr_count] - x),((double)y_wp[wpr_count] - y)) * 180.0/3.14159;
	proximity = sqrt(pow(((double)x_wp[wpr_count]-x), 2) + pow(((double)y_wp[wpr_count]-y), 2));
	return ;
}
...
void update_steering(){
	...
	if (angle_diff < -180.0) angle_diff += 360.0;   //if angle is less than 180 deg, then add 360 deg
	if (angle_diff > 180.0) angle_diff -= 360.0;	//if angle is greater than 180 deg, then subtract 360
	//now, we have an angle as -180 < angle_diff < 180. 
	steer_us = angle_diff/-360.0*STEER_GAIN;
...
void calculate_null(){
	...
	gyro_null = angle_long/gyro_count - 30;	//calculate the null. the -30 is a fudge factor for 5000 pts.
...
void read_FIFO(){
  ...
  for (int i=0; i < samplz; i++){
		accelgyro.getFIFOBytes(buffer, 2);
		angle_long -= ((((int16_t)buffer[0]) << 8) | buffer[1])*10 + gyro_null;
		gyro_count++;
		if ((angle_long > GYRO_CAL) && (!cal_flag)) angle_long -= GYRO_CAL; //if we are calculating null, don't roll-over
		if ((angle_long < 0) && (!cal_flag)) angle_long += GYRO_CAL;
		angle = angle_long*360.0/GYRO_CAL;
	}
