...
long count, proximity, previous_proximity=5000;
...
void setup(){
	...
	while(digitalRead(7)) get_mode();		//waits until the switch is flipped to start the race
	delay(2000);
	...
	while(!digitalRead(7) && (automatic)){	//waits for the switch to be flipped
		get_mode();		//waits until the switch is flipped to start the race
		read_FIFO();
	}
	for(int i=0; i<10; i++){	//waits 1 second before starting
		delay(1);
		read_FIFO();
	}
